...
packages/drizzle  @highjoon-dev/drizzle Drizzle ORM 스키마 & 클라이언트 & 마이그레이션 (DB 스키마 SSOT)
...
pnpm drizzle generate # Drizzle 마이그레이션 생성 (schema 변경 후)
```

패키지 필터 단축: `pnpm web <cmd>`, `pnpm ui <cmd>`, `pnpm drizzle <cmd>`
...
- **scope**: 선택. 도메인/슬라이스 단위로만 사용 (`category`, `post`, `api`, `drizzle` 등). 패키지 경로 표기(`(web)` 등)는 지양하고, 해당 패키지명은 설명에 녹인다 (예: `feat: drizzle ...`)
...
- **환경 변수**: 각 패키지의 `.env`를 Next.js / drizzle-kit CLI가 자동 로드
- **서버 액션**: Next.js server actions + `revalidatePath`로 캐시 무효화
- **서비스 레이어**: `entities/*/services/`, `features/*/services/` — Drizzle 기반 비즈니스 로직, 도메인별 폴더. 도메인 타입은 `entities/*/model/types.ts`에서 `typeof schema.x.$inferSelect`로 파생
...
- `DATABASE_URL` — Drizzle 연결 (PostgreSQL, Supabase pgbouncer 풀러)
