...
- **Backend**: Express.js, Prisma 6.3 (PostgreSQL)
...
```
apps/web          @highjoon-dev/web     Next.js 블로그 프론트엔드
apps/api          @highjoon-dev/api     Express REST API 서버
...
shared/    공유 유틸 (API 클라이언트, 라우트 상수, 페이지네이션, UI)
...
- **API 클라이언트**: `serverApi` (서버 컴포넌트용), `clientApi` (클라이언트 컴포넌트용) - `src/shared/api/`
