...
const TableOfContents = () => {
  ...
  return (
    <ul className="flex flex-col border-l-[1px] fixed" style={{ margin: 0, paddingLeft: '10px' }}>
      {headingElements.map((element, index) => (
        <li key={String(element) + index} className="list-none" style={{ margin: 0, paddingLeft: 0 }}>
          <Link
            href={`#${element.id}`}
            onClick={handleClickLink}
            className={`${
              element.id === activeId ? 'text-primary-500' : 'text-grey-900'
