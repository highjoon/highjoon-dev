...
반면 셀렉터를 전달하면 오직 셀렉터 함수의 반환값만을 구독합니다. 이는 꽤 강력합니다. 왜냐하면 할 일의 이름을 업데이트해도 `useTodosCount`를 통해 횟수를 구독하고 있는 컴포넌트는 리렌더링되지 않을 것이기 때문입니다. 횟수는 변하지 않았기 때문에 react-query는 이 옵저버에게 업데이트를 알리지 않아도 괜찮습니다. 🥳 (이 설명에서는 약간 간략하게 설명했습니다. 실제 기술적으로는 완전히 맞는 설명은 아닙니다. Part 3에서 렌더링 최적화에 대해서 더 설명하겠습니다.)
