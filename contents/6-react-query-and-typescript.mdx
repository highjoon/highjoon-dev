...
React Query는 처음에 v1에서 자바스크립트로 작성되었고, v2에서 타입스크립트로 재작성 되었습니다. 이는 React Query가 이제 타입스크립트 사용자들을 아주 잘 지원한다는 점을 의미합니다.
...
어쨌든 에러가 있는지 확인하기 위해 어떤 식으로든지간에 확인을 해야하기 때문에, instanceof 검사는 그렇게 나쁜 생각처럼 보이지 않습니다. 또한 이 검사는 에러가 런타임에 실제로 에러 메시지를 갖고 있는지도 확인합니다. 이는 타입스크립트가 4.4 릴리즈에서 도입할 예정인 새로운 컴파일러 플래그 `useUnknownInCatchVariables`궤를 같이합니다. 이로 인해 catch 변수가 `any` 대신 `unknown`이 될 것입니다. ([이 곳](https://github.com/microsoft/TypeScript/issues/41016)을 참고하세요.)
...
게다가 React Query는 모듈 확장을 통해 전역 에러를 등록하는 것으 허용합니다.
